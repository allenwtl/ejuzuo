...
(function ($, win, doc) {
    ...
    var uploadifyOption = {
        ...
        'onUploadSuccess':function(file, data){
            ...
            if("uploadifyCover" == id){
                ...
                $img = $("<img src='${resUrl}"+$data.path.pic280280+"'/>");
            } else {
                $imgInput = $("<input type='hidden' name='img' value="+JSON.stringify($data.path.picold)+" />");
                $img = $("<img src='${resUrl}"+$data.path.picold+"'/>");
            }
    ...
    var bindClickEvent = function(){
        ...
        $('input.delete').unbind().click(function(){
            console.info("sss");
            $(this).parents('div.deleteDiv').remove();
        });

        $('input.add').unbind().click(function(){
