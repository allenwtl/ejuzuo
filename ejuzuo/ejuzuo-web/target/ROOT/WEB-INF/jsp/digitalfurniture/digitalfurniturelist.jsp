            ...
            <div style="float: right;width: 1020px;">
              <c:forEach items="${brandList}" var="item" varStatus="idx" >
                <span class="pro_imgbox ${brand == item.id ? 'on':''}" data="${item.id}">
                  <img src="http://r.ejuzuo.com${item.logoImg}">
                </span>
              </c:forEach>
            </div>
...
<script type="text/template" id="dotTemplate">
  ...
  {{~it:value:index }}
  <li class="appendTo">
    <a href="/digital/digitalDetail/{{=value.id}}"><img src="http://r.ejuzuo.com{{=value.thumbnail}}" alt=""></a>
...
<script type="text/javascript" src="http://r.ejuzuo.com/js/digitalfurniture/digitalfurniturelist.js"></script>
</body>
</html>